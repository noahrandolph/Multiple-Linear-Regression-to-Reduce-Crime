...
##Model 1 alternate
```{r}
model1_a = lm(log(scaledcrime) ~ pctconv + log(density) + pctsen)
(AIC1 = AIC(model1_a))
plot(model1_a)
summary(model1_a)
vif(model1_a)
...
```{r}
model2_a = lm(log(scaledcrime) ~ pctconv + log(density) + scaledpolice+ pctsen  + pctmin)
AIC2 = AIC(model2_a)
plot(model2_a)
summary(model2_a)
vif(model2_a)
...
#Model 3 alt
```{r}
model3_a = lm(log(scaledcrime) ~ pctconv + log(density) + scaledpolice+ pctsen  + pctmin +pctarr + ymale + avgsen +  mix + avgwage + central + west +tax)
AIC3 = AIC(model3_a)
plot(model3_a)
summary(model3_a)
vif(model3_a)
...
```{r}
# Adjust standard errors
cov1 = vcovHC(model1_a)
robust_se1 = sqrt(diag(cov1))
cov2 = vcovHC(model2_a)
robust_se2 = sqrt(diag(cov2))
cov3 = vcovHC(model3_a)
robust_se3 = sqrt(diag(cov3))

# Adjust F statistic 
wald_results <- waldtest(model1_a, vcov = cov1)

stargazer(model1_a, model2_a, model3_a, type = 'text', intercept.bottom = FALSE,
          se        = list(robust_se1, robust_se2, robust_se3),
          omit.stat = "f",
          add.lines = list(c("AIC", round(AIC1, 2), round(AIC2, 2), 
                             round(AIC3, 2))))
