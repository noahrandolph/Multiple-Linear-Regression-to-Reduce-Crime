...
```{r}
scaledcrime = crime*100
scaledpolice = police*100

model1 = lm(scaledcrime ~ scaledpolice + probsen + probconv)
AIC1 = AIC(model1)
plot(model1)
...
```{r}
model2 = lm(scaledcrime ~ scaledpolice+ probsen + probconv + density + pctmin +
                    probconv)
AIC2 = AIC(model2)
plot(model2)
vif(model2)
...
```{r}
model3 = lm(scaledcrime ~ scaledpolice + probsen + probconv + density + pctmin +
                    probarr + ymale + avgsen +  mix + avgwage + central + west +
                    tax)
AIC3 = AIC(model3)
plot(model3)
vif(model3)
