...
```{r}
hist(Data$crime, breaks = 20, main = "County Crime Rates", xlab = "crimes committed per person")
```
...
hist(Data$probarr, breaks = 16, main = "Proportion of County Crimes Resulting in Arrests",
...
```{r}
hist(Data$probconv, breaks = 10, main = "Proportion Being Convicted After Arrest",
     xlab = "Proportion")
```
...
hist(Data$probsen, breaks = 20, main = "Proportion Being Sentenced After Conviction",
     xlab = "Proportion")
```
...
hist(Data$police, breaks = 20, main = "Police Per Capita",
...
hist(Data$density, breaks = 20, main = "Density",
...
```{r}
hist(Data$tax, breaks = 20)
```


```{r}
hist(Data$ymale, breaks = 20)
```

```{r}
hist(Data$pctmin, breaks = 20)
```

```{r}
scatterplotMatrix(~ log(Data$ymale) + log(Data$police) + Data$pctmin +
                          Data$density)
```

```{r}
scatterplotMatrix(~ log(Data$police) + Data$probarr + Data$probconv +
                          Data$probsen)
```

```{r}
plot(jitter(Data$urban), jitter(Data$west))
```

```{r}
plot(jitter(Data$urban), jitter(Data$central))
```

#### Model 1

```{r}
scaledcrime = Data$crime*100

model1 = lm(scaledcrime ~ Data$probsen + log(Data$police))
AIC1 = AIC(model1)
plot(model1)
...
```{r}
model2 = lm(scaledcrime ~ Data$density + log(Data$ymale) + Data$probsen +
                    log(Data$police) + Data$pctmin)
AIC2 = AIC(model2)
plot(model2)
vif(model2)
```
...
```{r}
model3 = lm(scaledcrime ~ Data$tax + Data$density + log(Data$ymale) +
                    Data$probarr + Data$probconv + Data$probsen + Data$avgsen +
                    Data$pctmin + Data$mix + log(Data$police) + avgwage +
                    Data$central + Data$west)
AIC3 = AIC(model3)
plot(model3)
vif(model3)
```
